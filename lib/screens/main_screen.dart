...
class _MainScreenState extends State<MainScreen> {
  ...
  DemoPaywallFetchPolicy _examplePaywallFetchPolicy =
      DemoPaywallFetchPolicy.reloadRevalidatingCacheData;
  ...
  void _subscribeForEvents() {
    observer.onAdaptyErrorOccurred = (error) {
      ...
      _showErrorDialog(
          'Adapty Error ${error.code}', error.message, error.detail);
    };
  ...
  Widget _buildProfileIdSection() {
    return ListSection(
      ...
      children: [
        ListActionTile(
          title: this.adaptyProfile != null
              ? '${this.adaptyProfile!.profileId}'
              : 'null',
          isActive: this.adaptyProfile != null,
          onTap: () {
            Clipboard.setData(
                ClipboardData(text: this.adaptyProfile!.profileId));
          },
        ),
      ],
    );
  ...
  Widget _buildIdentifySection() {
    return ListSection(
      ...
      footerText: null,
      children: [
        if (adaptyProfile?.customerUserId != null)
          ListTextTile(
              title: 'Current', subtitle: adaptyProfile!.customerUserId!),
        ListTextFieldTile(
          ...
          onSubmitted: (txt) {
            setState(() {
              _enteredCustomerUserId = txt;
              if (_enteredCustomerUserId != null &&
                  _enteredCustomerUserId!.isNotEmpty) {
    ...
    return ListSection(
      ...
      children: [
        ListTextTile(
          ...
          subtitleColor: (premium?.isActive ?? false)
              ? CupertinoColors.systemGreen
              : CupertinoColors.systemRed,
        ),
        ListTextTile(
            title: 'Is Lifetime',
            subtitle: (premium?.isLifetime ?? false) ? 'true' : 'false'),
        if (premium != null)
          ListTextTile(
              title: 'Activated At',
              subtitle: _dateTimeFormattedString(premium.activatedAt)),
        if (premium != null && premium.renewedAt != null)
          ListTextTile(
              title: 'Renewed At',
              subtitle: _dateTimeFormattedString(premium.renewedAt!)),
        if (premium != null && premium.expiresAt != null)
          ListTextTile(
              title: 'Expires At',
              subtitle: _dateTimeFormattedString(premium.expiresAt!)),
        ListTextTile(
            title: 'Will Renew',
            subtitle: (premium?.willRenew ?? false) ? 'true' : 'false'),
        ListTextTile(
            title:
                'Subscriptions: ${adaptyProfile?.subscriptions.length ?? 0}'),
        ListTextTile(
            title:
                'NonSubscriptions: ${adaptyProfile?.nonSubscriptions.length ?? 0}'),
          ...
          ListActionTile(
            ...
            onTap: () {
              Navigator.of(context).push(CupertinoPageRoute(
                  builder: (ctx) => PaywallScreen(paywall: paywall)));
            },
          ),
        ],
      );
  ...
  DemoPaywallFetchPolicy _customPaywallFetchPolicy =
      DemoPaywallFetchPolicy.reloadRevalidatingCacheData;
  ...
  Widget _buildCustomPaywallSection() {
    return ListSection(
      ...
      footerText:
          'Here you can load any paywall by its id and inspect the contents',
      children: [
        ...
        if (_customPaywall != null) ...[
          ListTextTile(
              title: 'Paywall Id', subtitle: _customPaywall!.placementId),
          ..._paywallContents(
            _customPaywall!,
  ...
  Future<void> _updateAttribution() async {
    ...
    await observer.callUpdateAttribution({'key1': 'value1', 'key2': 'value2'},
        AdaptyAttributionSource.custom, '123456');
...
extension DemoPaywallFetchPolicyExtension on DemoPaywallFetchPolicy {
  ...
  AdaptyPaywallFetchPolicy adaptyPolicy() {
    switch (this) {
      ...
      case DemoPaywallFetchPolicy
            .returnCacheDataIfNotExpiredElseLoadMaxAge10sec:
        return AdaptyPaywallFetchPolicy.returnCacheDataIfNotExpiredElseLoad(
