...
class PurchasesObserver {
  ...
  Future<Map<String, AdaptyEligibility>?>
      callGetProductsIntroductoryOfferEligibility(
          List<AdaptyPaywallProduct> products) async {
    try {
  ...
  Future<void> callUpdateAttribution(Map<dynamic, dynamic> attribution,
    ...
    try {
      await adapty.updateAttribution(attribution,
          source: source, networkUserId: networkUserId);
    } on AdaptyError catch (adaptyError) {
      onAdaptyErrorOccurred?.call(adaptyError);
    } catch (e) {
      onUnknownErrorOccurred?.call(e);
  ...
  Future<void> callLogShowOnboarding(
      String? name, String? screenName, int screenOrder) async {
    try {
      await adapty.logShowOnboarding(
          name: name, screenName: screenName, screenOrder: screenOrder);
    } on AdaptyError catch (adaptyError) {
      onAdaptyErrorOccurred?.call(adaptyError);
    } catch (e) {
      onUnknownErrorOccurred?.call(e);
